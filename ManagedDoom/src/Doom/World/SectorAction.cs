    ...
    public sealed class SectorAction
    {
        ...
        private bool CrushThing(Mobj thing)
        {
            ...
            if (crushChange && (world.LevelTime & 3) == 0)
            {
                ...
                Mobj blood = world.ThingAllocation.SpawnMobj(
                    thing.X,
                    thing.Y,
        ...
        private bool ChangeSector(Sector sector, bool crunch)
        {
            ...
            int[] blockBox = sector.BlockBox;

            // Re-check heights for all things near the moving sector.
            for (int x = blockBox.Left(); x <= blockBox.Right(); x++)
            {
                for (int y = blockBox.Bottom(); y <= blockBox.Top(); y++)
                {
                ...
                case 0:
                    ...
                    switch (direction)
                    {
                        case -1:
                            ...
                            {
                                Fixed lastPos = sector.FloorHeight;
                ...
                case 1:
                    ...
                    switch (direction)
                    {
                        case -1:
                            ...
                            {
                                Fixed lastPos = sector.CeilingHeight;
        ...
        {
            Fixed floor = sector.FloorHeight;

            for (int i = 0; i < sector.Lines.Length; i++)
            {
                LineDef check = sector.Lines[i];

                Sector? other = GetNextSector(check, sector);
                if (other == null)
                {
                    continue;
                }
        ...
        public void DoLocalDoor(LineDef line, Mobj thing)
        {
            ...
            Sector sector = line.BackSide.Sector;
        ...
        public bool DoDoor(LineDef line, VerticalDoorType type)
        {
            ...
            {
                Sector sector = sectors[setcorNumber];
                if (sector.SpecialData != null)
                {
                    continue;
                }
        ...
        {
            for (int j = 0; j < activePlatforms.Length; j++)
            {
            ...
            while ((sectorNumber = FindSectorFromLineTag(line, sectorNumber)) >= 0)
            {
                ...
                switch (type)
                {
                    ...
                    case FloorMoveType.RaiseToTexture:
                        ...
                        floor.Sector = sector;
                        floor.Speed = floorSpeed;
                        ITextureLookup textures = world.Map.Textures;
                        for (int i = 0; i < sector.Lines.Length; i++)
                        {
                            if ((sector.Lines[i].Flags & LineFlags.TwoSided) != 0)
                            {
                                SideDef? frontSide = sector.Lines[i].FrontSide;
                                ...
                                SideDef? backSide = sector.Lines[i].BackSide;
        ...
        public bool BuildStairs(LineDef line, StairType type)
        {
            ...
            while ((sectorNumber = FindSectorFromLineTag(line, sectorNumber)) >= 0)
            {
                ...
                floor.Speed = speed;
                Fixed height = sector.FloorHeight + stairSize;
                floor.FloorDestHeight = height;

                int texture = sector.FloorFlat;
                ...
                do
                {
                    ...
                    for (int i = 0; i < sector.Lines.Length; i++)
                    {
                        ...
                        Sector? target = (sector.Lines[i]).FrontSector;
                        int newSectorNumber = target.Number;
            ...
            for (int i = 0; i < sectors.Length; i++)
            {
                if (sectors[i].Tag == tag)
                {
                    foreach (Thinker thinker in world.Thinkers)
                    {
                        ...
                        Sector sector = dest.Subsector.Sector;
        ...
        public void TurnTagLightsOff(LineDef line)
        {
            ...
            for (int i = 0; i < sectors.Length; i++)
            {
                ...
                if (sector.Tag == line.Tag)
                {
                    ...
                    {
                        Sector? target = GetNextSector(sector.Lines[j], sector);
                        if (target == null)
                        {
                            continue;
                        }
        ...
        public bool DoDonut(LineDef line)
        {
            ...
            while ((sectorNumber = FindSectorFromLineTag(line, sectorNumber)) >= 0)
            {
                Sector s1 = sectors[sectorNumber];
                ...
                for (int i = 0; i < s2.Lines.Length; i++)
                {
